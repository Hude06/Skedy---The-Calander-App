...
const createCapacitor = (win) => {
  ...
  const defaultRegisterPlugin = (pluginName, jsImplementations = {}) => {
    ...
    const addListenerNative = (eventName2, callback) => {
      const call = addListener({ eventName: eventName2 }, callback);
      const remove = async () => {
...
class WebPlugin {
  ...
  addListener(eventName2, listenerFunc) {
    ...
    this.listeners[eventName2].push(listenerFunc);
    const windowListener = this.windowListeners[eventName2];
    if (windowListener && !windowListener.registered) {
      this.addWindowListener(windowListener);
    }
    if (firstListener) {
      this.sendRetainedArgumentsForEvent(eventName2);
    }
    const remove = async () => this.removeListener(eventName2, listenerFunc);
  ...
  notifyListeners(eventName2, data, retainUntilConsumed) {
    const listeners = this.listeners[eventName2];
    if (!listeners) {
      if (retainUntilConsumed) {
        let args = this.retainedEventArguments[eventName2];
        if (!args) {
          args = [];
        }
        args.push(data);
        this.retainedEventArguments[eventName2] = args;
      }
      return;
    }
    listeners.forEach((listener) => listener(data));
  }
  hasListeners(eventName2) {
    return !!this.listeners[eventName2].length;
  }
  ...
  async removeListener(eventName2, listenerFunc) {
    const listeners = this.listeners[eventName2];
    if (!listeners) {
      return;
    }
    const index = listeners.indexOf(listenerFunc);
    this.listeners[eventName2].splice(index, 1);
    if (!this.listeners[eventName2].length) {
      this.removeWindowListener(this.windowListeners[eventName2]);
    }
  }
  ...
  sendRetainedArgumentsForEvent(eventName2) {
    const args = this.retainedEventArguments[eventName2];
    if (!args) {
      return;
    }
    delete this.retainedEventArguments[eventName2];
    args.forEach((arg) => {
      this.notifyListeners(eventName2, arg);
    });
  }
}
...
calander.addEventListener("click", function() {
  console.log("click");
  calanderVisable = !calanderVisable;
  if (calanderVisable) {
    document.getElementById("tasks").style.zIndex = 6;
    document.getElementById("calanderMenu").style.zIndex = 7;
  } else {
    document.getElementById("tasks").style.zIndex = 7;
    document.getElementById("calanderMenu").style.zIndex = 6;
  }
});
